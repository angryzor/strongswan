	...
	private_oqs_qske_t *this, chunk_t value)
{
	...
	{
		DBG1(DBG_LIB, "wrong %N public key size",
			 qske_mechanism_names, this->qske_mechanism);
		return FALSE;
	}
	free(this->public_key);
	this->public_key = strndup(value.ptr, value.len);
	...
	private_oqs_qske_t *this, chunk_t value)
{
	...
	{
		DBG1(DBG_LIB, "wrong %N ciphertext size",
			 qske_mechanism_names, this->qske_mechanism);
		return FALSE;
	}
	free(this->ciphertext);
	this->ciphertext = strndup(value.ptr, value.len);
	...
	rc = OQS_KEM_decaps(this->kem, this->shared_secret, this->ciphertext,
						this->secret_key);
	if (rc != OQS_SUCCESS)
	{
 ...
 * Described in header.
 */
oqs_qske_t *oqs_qske_qske_create(qske_mechanism_t mechanism)
{
	private_oqs_qske_t *this;
	char *kem_alg = NULL;
	OQS_KEM *kem;
	...
	switch (mechanism)
	{
		...
		case QSKE_BIG_QUAKE_L1:
			kem_alg = OQS_KEM_alg_BIG_QUAKE_1;
			break;
		case QKSE_BIG_QUAKE_L3:
			kem_alg = OQS_KEM_alg_BIG_QUAKE_3;
			break;
		case QKSE_BIG_QUAKE_L5:
			kem_alg = OQS_KEM_alg_BIG_QUAKE_5;
			break;
		case QSKE_SABER_L1:
